...
export async function salvarConfiguracao(config: ConfiguracaoEscala): Promise<void> {
  const dados = await carregarDados();
  await salvarDados({ ...dados, escala: JSON.stringify(config) });
}

export async function carregarConfiguracao(): Promise<ConfiguracaoEscala> {
  const dados = await carregarDados();
  const config = dados.escala;
